...
def add_to_order():
    ...
    try:
        ...
        so_luong_ton_kho = sanpham[0]

        # Kiểm tra nếu số lượng yêu cầu vượt quá số lượng tồn kho
        if so_luong > so_luong_ton_kho:
            flash('Số lượng sản phẩm yêu cầu vượt quá số lượng tồn kho.', 'error')
            return redirect(url_for('quan_ly_ban.chitiethoadon', ma_ban=request.form['MaBan']))
        ...
        connection.commit()
        flash('Sản phẩm đã được thêm vào đơn hàng.', 'success')
        ...
        cursor.close()
        connection.close()
        flash('Đã xảy ra lỗi khi thêm sản phẩm.', 'error')
        return redirect(url_for('quan_ly_ban.quanlybillards'))
...
def tao_hoadon():
    ...
    if table:
        cursor.execute("INSERT INTO hoadon (MaBan, ThoiGianBatDau) VALUES (%s, NOW())", (MaBan,))
        connection.commit()

        new_invoice_id = cursor.lastrowid
        
        cursor.execute("UPDATE banbida1 SET TinhTrang = 'Có khách' WHERE MaBan = %s", (MaBan,))
        connection.commit()
...
def select_member(invoice_id):
    ...
    return render_template('/quan_li_ban_bia/select_member.html', hoadon=hoadon, members=members)
